...
bool lookup(string s)
{
    // TODO
    char s1[10];
    int first=0, last = dictionary.size-1, mid, i;
    for(i = 0; s[i]!='\0'; i++)
        s1[i] = s[i];
    s1[i] = '\0';
    while(first <= last)
    {
        mid = (first+last)/2;
        if( strcmp(dictionary.words[mid].letters,s1) == 0 && !dictionary.words[mid].found)
        {
            dictionary.words[mid].found=true;
            return true;
        }
        else if( strcmp(dictionary.words[mid].letters,s1) > 0 )
            last = mid-1;
        else
            first = mid+1;
    }
